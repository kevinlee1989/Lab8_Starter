  ...
  if('serviceWorker' in navigator){
    ...
    window.addEventListener('load', async () => {
      try{
        ...
        console.log('Service working is now registered and ready to go', registration);

      } catch (error){
        // B5. TODO - In the event that the service worker registration fails, console
        //            log that it has failed.
        console.log('Service worker failed to work ', error);
      }
    });
...
async function getRecipes() {
  ...
  //            function is using the async keyword so we recommend "await"
      console.log(`Fetching: ${RECIPE_URLS[i]}`);
  ...
  //            resolve() method.
      if(recipes.length === RECIPE_URLS.length){
        ...
        console.error(`Failed to fetch recipe from ${RECIPE_URLS[i]}`, error);
